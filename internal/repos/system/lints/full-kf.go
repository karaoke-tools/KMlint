...
func FullKf() lint.Lint {
	return lint.Lint{
		Name:        "full-kf",
		Description: "lyrics with a lot of kf",
		...
		RunFunc: func(ctx context.Context, KaraData karadata.KaraData) (report.Report, error) {
			...
			if FULL_KF_CRITICAL_RATIO_KF*kf_count >= FULL_KF_CRITICAL_RATIO_K*k_count {
				return report.FailCritical("too many \\kf in the song"), nil
			}
			if FULL_KF_WARNING_RATIO_KF*kf_count >= FULL_KF_WARNING_RATIO_K*k_count {
				return report.FailWarning("too many \\kf in the song"), nil
			}
